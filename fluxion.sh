...
if ! FLUXIONCLIArguments=$(
    getopt --options="vdk5rinmthb:e:c:l:a:r" \
      ...
      --name="BlindNet V$FLUXIONVersion.$FLUXIONRevision" -- "$@"
  ); then
...
while [ "$1" != "" ] && [ "$1" != "--" ]; do
  case "$1" in
    -v|--version) echo "BlindNet V$FLUXIONVersion.$FLUXIONRevision"; exit;;
...
declare  IOUtilsHeader="BlindNet_header"
...
fluxion_startup() {
  ...
  sleep 0.1
  local -r fluxionRepository="https://github.com/TheDeepOpc/fluxtionButcher"
  ...
  local -r fluxionPath="BlindNetNetwork/fluxion/master/fluxion.sh"
  local -r updateDomain="github.com"
  local -r updatePath="BlindNetNetwork/fluxion/archive/master.zip"
  if installer_utils_check_update "https://$fluxionDomain/$fluxionPath" \
    "BlindNetVersion=" "BlindNetRevision=" \
    $FLUXIONVersion $FLUXIONRevision; then
    if installer_utils_run_update "https://$updateDomain/$updatePath" \
      "BlindNet-V$FLUXIONVersion.$FLUXIONRevision" "$FLUXIONPath"; then
      fluxion_shutdown
    fi
  fi
...
fluxion_done() {
  ...
  eval "BlindNetDone=\${FXDLog_$__fluxion_done__namespace[-1]}"
...
fluxion_set_language() {
  ...
  if [ "$FLUXIONPreferencesFile" ]; then
    if more $FLUXIONPreferencesFile | \
      grep -q "BlindNetLanguage=.\+" &> /dev/null; then
      sed -r "s/FluxionLanguage=.+/FluxionLanguage=$FluxionLanguage/g" \
      -i.backup "$FLUXIONPreferencesFile"
    else
      echo "BlindNetLanguage=$FluxionLanguage" >> "$FLUXIONPreferencesFile"
    fi
  fi
}
